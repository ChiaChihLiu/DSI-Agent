...
SELECT
    ...
    -- 採購調整建議（%）
    CASE
        WHEN variance_pct > 20 THEN CONCAT('+', ROUND(variance_pct / 2, 0), '% 增加採購')
        WHEN variance_pct < -20 THEN CONCAT(ROUND(variance_pct / 2, 0), '% 減少採購')
        ELSE '維持現況'
    END as 採購調整建議,
    -- OTB影響（若有超額/不足）
    CASE
        WHEN variance_pct < -20 THEN
            CONCAT('⚠️ 凍結OTB額度: ', ROUND(ABS(variance), 0), ' units')
        WHEN variance_pct > 20 THEN
            CONCAT('💡 額外OTB需求: ', ROUND(variance / 2, 0), ' units')
        ELSE NULL
    END as OTB影響,
    -- 優先級
    CASE
